...
const createAccountValidation = [
  body("platform").isIn(Object.values(Platform)).withMessage("Valid platform is required"),
  body("username").trim().isLength({ min: 1, max: 100 }).withMessage("Username is required and must be max 100 characters"),
  body("displayName").optional().trim().isLength({ max: 100 }).withMessage("Display name must be max 100 characters"),
];

const updateAccountValidation = [
  body("username").optional().trim().isLength({ min: 1, max: 100 }).withMessage("Username must be max 100 characters"),
  body("displayName").optional().trim().isLength({ max: 100 }).withMessage("Display name must be max 100 characters"),
];
 ...
 * @access Private
 */
router.post("/", authenticate, createAccountValidation, async (req: Request, res: Response) => {
  try {
    ...
    if (platform !== Platform.INSTAGRAM) {
      throw new ApiError("Only Instagram accounts are currently supported", 400);
    }
    ...
    try {
      const testData = await apifyService.instance.scrapeInstagramProfile(username);
      ...
      logger.error(`Failed to validate Instagram account: ${username}`, scrapeError);
      throw new ApiError("Unable to validate Instagram account. Please check the username and try again.", 400);
    }
  } catch (error) {
    if (error instanceof ApiError) {
      res.status(error.statusCode).json({
 ...
 * @access Private
 */
router.get("/:id", authenticate, accountIdValidation, async (req: Request, res: Response) => {
  try {
    checkValidationErrors(req);
 ...
 * @access Private
 */
router.put("/:id", authenticate, accountIdValidation, updateAccountValidation, async (req: Request, res: Response) => {
  try {
    checkValidationErrors(req);
    ...
    const account = await socialAccountService.updateAccount(accountId, userId, updates);
 ...
 * @access Private
 */
router.delete("/:id", authenticate, accountIdValidation, async (req: Request, res: Response) => {
  try {
    checkValidationErrors(req);
